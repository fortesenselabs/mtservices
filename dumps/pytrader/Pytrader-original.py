...
TZ_UTC    = 'UTC'
SOCKET_BUFFER =  4096 # 1000000 # 4096
SOCKET_BUFFER_COMMAND = 500000
...
ERROR_DICT = {
    ...
    '02004': 'Unknown instrument for broker',
    '02003': 'Unknown instrument for broker',
    '02101': 'Instrument not in demo',
    '02102': 'No ticks',
    '02103': 'Not implemented in MT4',
    ...
    '07011': 'Unknown instrument for broker',
    ...
    '09103': 'Function not implemented/unknown command',
    '09104': 'empty command',
    '99900': 'Wrong authorization code',
...
class PyTraderAPI:             
    ...
    def Connect(self,
                ...
                authorization_code: str = '1234') -> bool:
        """
        ...
        Args:
            ...
            port: port number
            instrument_lookup: list with general instrument names and broker instrument names
        ...
        try:
            ...
            try:
                data_received = self.sock.recv(SOCKET_BUFFER)
                self.connected = True
                self.socket_error = 0
    ...
    def Get_account_info(self) -> dict:
        ...
        ok, dataString = self.send_command('ACCOUNT_INFO')
        if (ok == False):
            self.command_OK = False
            return None
        
        print(respDataString)

        respDataString = self.parse_terminal_response(dataString)
        if respDataString is None:
            return None
        ...
        returnDict['name'] = str(respDataString[0])
        returnDict['login'] = str(respDataString[1])
        returnDict['currency'] = str(respDataString[2])
        returnDict['type'] = str(respDataString[3])
        returnDict['leverage'] = int(respDataString[4])
        returnDict['trade_allowed'] = bool(respDataString[5])
        returnDict['limit_orders'] = int(respDataString[6])
        returnDict['margin_call'] = float(respDataString[7])
        returnDict['margin_close'] = float(respDataString[8])
        returnDict['company'] = str(respDataString[9])
    ...
    def send_command(self, command):
        self.command: str = command + "|" + self.authorization_code + "\r\n"
        self.timeout: bool = False
        #print(self.command)
        #print(self.socket)
        ...
        try:
            # self.sock.send(bytes(self.command, "utf-8"))
            self.sock.send(self.command.encode())

            data_received = self._get_reply() #''
            # while True:
            #     data_received += self.sock.recv(SOCKET_BUFFER)
    ...
    def _get_reply(self):
        ...
        try:
            ...
            while True:
                ...
                buffer += data
                try:
                    msg, idx = buffer.decode()
                    buffer = buffer[idx:]
                    return msg  # Return the parsed string
                except ValueError:
                    pass
    ...
    def parse_terminal_response(self, data: str):
        if self.debug:
            print(data)

        self.command_return_error = ''
            
        if data.find('error=true') != -1:
            self.command_OK = False
            code = data.split("|")[1].split("=")[1]
            self.command_return_error = ERROR_DICT[code]
