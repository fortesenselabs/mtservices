...
from common.app_base_template import BaseApp
from common.utils import *
from apscheduler.schedulers.background import BackgroundScheduler


log = logging.getLogger('binance_collector_websockets')
...
class BinanceCollectorWebSockets():
    ...
    def start_collector(self):
        ...
        # List of streams
        channels = self.config["collector"]["stream"]["channels"]
        print(f"Channels: {channels}")

        # List of symbols
        symbols = self.config["collector"]["stream"]["symbols"]
        ...
        self.bm = BinanceSocketManager(self.client, user_timeout=BinanceSocketManager.DEFAULT_USER_TIMEOUT)
        self.conn_key = self.bm.start_multiplex_socket(streams, self.process_message)
        self.bm.start()
        ...
        saving_period = self.config["collector"]["flush_period"]
        try:
            while True:
                time.sleep(saving_period)
                event_count = self.analyzer.queue.qsize()
                if event_count > 0:
                    print(f"Storing {event_count} events.")
                    self.analyzer.store_queue()
                else:
                    # Reconnect
