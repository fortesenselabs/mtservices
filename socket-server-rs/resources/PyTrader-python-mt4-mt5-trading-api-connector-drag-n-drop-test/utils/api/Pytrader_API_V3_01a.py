...
class Pytrader_API:
    ...
    def Get_static_account_info(self) -> dict:
        ...
        returnDict = {
            ...
            'type': str(x[5]),
            'leverage': int(x[6]),
            'trade_allowed': bool(int(x[7])),
            'limit_orders': int(x[8]),
            'margin_call': float(x[9]),
            'margin_close': float(x[10]),
            'company': str(x[11])
        }
    ...
    def Get_PnL(self,
                date_from: datetime = datetime(
                    2021, 3, 1, tzinfo=pytz.timezone("Etc/UTC")),
        ...
        closed_positions = self.Get_closed_positions_within_window(
            date_from, date_to)
        if type(closed_positions) == pd.DataFrame:
            ...
            unrealized_profit = dynamic_info['equity'] - \
                dynamic_info['balance']
            result = {}
    ...
    def Get_instrument_info(self,
        ...
        self.instrument = self.get_broker_instrument_name(
            self.instrument_name_universal)
    ...
    def Get_last_x_ticks_from_now(self,
        ...
        if (self.nbrofticks > self.max_ticks):
            ...
            for index in range(0, iloop):
                self.command = 'F021^4^' + self.instrument + '^' + \
                    str(index * self.max_ticks) + \
                    '^' + str(self.max_ticks) + '^'
                ok, dataString = self.send_command(self.command)
                if not ok:
                    self.command_OK = False
            ...
            if (itail > 0):
                self.command = 'F021^4^' + self.instrument + '^' + \
                    str(iloop * self.max_ticks) + '^' + str(itail) + '^'
                ok, dataString = self.send_command(self.command)
                if not ok:
                    self.command_OK = False
                    return None
    ...
    def Get_specific_bar(self,
        ...
        for index in range(0, len(instrument_list), 1):
            _instr = self.get_broker_instrument_name(
                instrument_list[index].upper())
            if (self.instrument == 'none' or self.instrument == None):
                self.command_return_error = 'Instrument not in broker list'
                self.command_OK = False
                return None
        ...
        self.command = self.command + '^' + \
            str(specific_bar_index) + '^' + str(timeframe) + '^'
        ok, dataString = self.send_command(self.command)
        ...
        if (self.numberofbars > self.max_bars):
            ...
            for index in range(0, iloop):
                self.command = 'F042^5^' + self.instrument + '^' + \
                    str(timeframe) + '^' + str(index * self.max_bars) + \
                    '^' + str(self.max_bars) + '^'
                ok, dataString = self.send_command(self.command)
                if not ok:
                    self.command_OK = False
        ...
        if ok == True and resp[0:5] == "F065^" and resp[-1] == "!":
            ...
            if int(nbr) != 0:
                df = pd.read_table(io.StringIO(resp[resp.index('^', 5)+1:-1]), sep='$', lineterminator='^',
                                   header=None,
                                   names=np.array(self.columnsDeletedOrders[1:])[
                    :, 0],
    ...
    def Get_deleted_orders_within_window(self,
                                         date_from: datetime = datetime(
                                             2021, 3, 25, tzinfo=pytz.timezone("Etc/UTC")),
                                         date_to: datetime = datetime.now()) -> pd.DataFrame:
        """ 
    ...
    def Get_all_orders(self) -> pd.DataFrame:
        ...
        if ok == True and resp[0:5] == "F060^" and resp[-1] == "!":
            ...
            if int(nbr) != 0:
                df = pd.read_table(io.StringIO(resp[resp.index('^', 5)+1:-1]), sep='$', lineterminator='^',
                                   header=None,
                                   names=np.array(self.columnsOpenOrders[1:])[
                    :, 0],
    ...
    def Get_all_open_positions(self) -> pd.DataFrame:
        ...
        if ok == True and resp[0:5] == "F061^" and resp[-1] == "!":
            ...
            if int(nbr) != 0:
                df = pd.read_table(io.StringIO(resp[resp.index('^', 5)+1:-1]), sep='$', lineterminator='^',
                                   header=None,
                                   names=np.array(self.columnsOpenPositions[1:])[
                    :, 0],
    ...
    def Get_closed_positions_within_window(self,
                                           date_from: datetime = datetime(
                                               2021, 3, 20, tzinfo=pytz.timezone("Etc/UTC")),
                                           date_to: datetime = datetime.now()) -> pd.DataFrame:
        """ 
        ...
        if ok == True and resp[0:5] == "F062^" and resp[-1] == "!":
            ...
            if int(nbr) != 0:
                df = pd.read_table(io.StringIO(resp[resp.index('^', 5)+1:-1]), sep='$', lineterminator='^',
                                   header=None,
                                   names=np.array(self.columnsClosedPositions[1:])[
                    :, 0],
    ...
    def Open_order(self,
        ...
        comment.replace('$', '')
        comment.replace('!', '')
        broker_instrument = self.get_broker_instrument_name(
            self.instrument_name_universal)
        if (self.instrument == 'none' or self.instrument == None):
        ...
        self.command = 'F070^11^' + self.get_broker_instrument_name(self.instrument_name_universal) + '^' + ordertype + '^' + str(volume) + '^' + \
            str(openprice) + '^' + str(slippage) + '^' + str(magicnumber) + '^' + \
            str(stoploss) + '^' + str(takeprofit) + '^' + \
            str(comment) + '^' + str(market) + '^'
        # print(self.command)
        ok, dataString = self.send_command(self.command)
        if not ok:
            self.command_OK = False
    ...
    def CloseBy_position_by_ticket(self,
        ...
        self.command_return_error = ''
        self.command = 'F074^3^' + \
            str(ticket) + '^' + str(ticket_opposite) + '^'
        ok, dataString = self.send_command(self.command)
        if not ok:
            self.command_OK = False
            return False
    ...
    def Get_logfile(self, date: datetime = datetime.now()) -> pd.DataFrame():
        ...
        if ok == True and resp[0:5] == "F082^" and resp[-1] == "!":
            ...
            if int(nbr) != 0:
                df = pd.read_table(io.StringIO(resp[resp.index('^', 5)+1:-1]), sep='$', lineterminator='^',
                                   header=None,
                                   names=np.array(self.columnsLogInfo[1:])[
                    :, 0],
        ...
        try:
            ...
            try:
                while not data_received.endswith('!'):
                    data_received += self.sock.recv(500000).decode()
            except socket.error as msg:
                self.connected = False
